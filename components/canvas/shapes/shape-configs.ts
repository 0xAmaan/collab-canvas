  ...
  polygon: {
    ...
    extractData: (polygon: Polygon, userId: string) => {
      ...
      return {
        ...
        fillColor: (polygon.fill as string) || DEFAULT_SHAPE.FILL_COLOR,
        x: polygon.left || 0,
        y: polygon.top || 0,
  ...
  path: {
    ...
    extractData: (path: Path, userId: string) => {
      ...
      return {
        ...
        fillColor: (path.stroke as string) || DEFAULT_SHAPE.FILL_COLOR, // Store stroke as fillColor for metadata
        createdBy: userId,
        createdAt: Date.now(),
        lastModified: Date.now(),
        lastModifiedBy: userId,
